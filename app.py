        ...
        try:
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    new_documents_from_file.append(Document(page_content=doc_text, metadata={"unique_id": doc_id}))
                except Exception as e:
                    print(f"    - WARNING: Skipping a row in {filename}. Reason: {e}")
                    continue
            
            # --- THE FIX: Process and add documents PER FILE ---
            if new_documents_from_file:
                try:
                    existing_ids = set(vector_store.get(include=["metadatas"])['ids'])
                except Exception:
                    existing_ids = set()
                
                unique_new_docs_map = {doc.metadata["unique_id"]: doc for doc in new_documents_from_file}
            ...
            # CRUCIAL: Always clean up the processed file
            try:
                os.remove(filepath)
